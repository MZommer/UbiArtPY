...
class UafSecureFat:
                ...
                for i, file in enumerate(files):
                    if file in self.bundles:
                        self.root.after(0, messagebox.showwarning, "Warning",
                                        f"Bundle {os.path.basename(file)} is already added.")
                        continue

                    logging.debug(f"Loading bundle: {file}")
                    ...
                    self.root.after(0, self.update_progress, i + 1, len(files))

                # Refresh the bundle tree in the main thread
                self.root.after(0, self.refresh_bundle_tree)
                self.root.after(0, self.update_engine_info)
        except Exception as e:
            self.root.after(0, messagebox.showerror, "Error", f"Failed to load bundle: {e}")
    ...
    def reload_bundle_files(self, bundle):
        ...
        try:
            with PackFile(bundle, "r") as unpacker:
                self.files[bundle] = {f: f.get_string_id() for f in unpacker.files}
            self.refresh_bundle_tree()
            self.root.after(0, messagebox.showinfo, "Success",
                            f"Bundle {os.path.basename(bundle)} reloaded successfully.")
        except Exception as e:
            logging.error(f"Error reloading bundle: {e}", exc_info=True)
            self.root.after(0, messagebox.showerror, "Error", f"Failed to reload bundle: {e}")
    ...
    def save_securefat(self):
        ...
        try:
            ...
            self.root.after(0, messagebox.showinfo, "Success", f"Secure FAT saved successfully to {destination}")
        except Exception as e:
            logging.error(f"Error saving Secure FAT: {e}", exc_info=True)
            self.root.after(0, messagebox.showerror, "Error", f"Failed to save Secure FAT: {e}")
...
if __name__ == "__main__":
    try:
        ...
        messagebox.showerror("Fatal Error", f"The application crashed: {e}")
