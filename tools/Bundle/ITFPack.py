...
from UbiArtPY import PackFile, Platform, PLATFORMS, Path, JdVersion, Versioning, String8
...
def pack_folder(source: Path, destination: Path, platform: Platform, jd_version: JdVersion):
    ...
    with PackFile(destination, "w", platform) as packer:
        ...
        for directory, _, files in os.walk(source):
            ...
            for file in files:
                file = Path(file)
                full_path = directory.copy_and_append_path(file)
                itf_path = Path(String8(full_path).replace(str(root) + "/", ""))
...
class PackUnpackApp:
    ...
    def create_pack_tab(self):
        ...
        ttk.Button(self.pack_tab, text="Browse", command=self.browse_pack_folder).grid(row=0, column=2, pady=10,
                                                                                       padx=10)
        ...
        ttk.Button(self.pack_tab, text="Browse", command=self.browse_pack_destination).grid(row=1, column=2, pady=10,
                                                                                            padx=10)
        ...
        self.platform_menu = ttk.Combobox(self.pack_tab, textvariable=self.platform_var,
                                          values=[p.name for p in PLATFORMS], state="readonly")
        ...
        ttk.Button(self.pack_tab, text="Pack Folder", command=self.pack_folder_thread).grid(row=4, column=0,
                                                                                            columnspan=3, pady=5)
    ...
    def create_unpack_tab(self):
        ...
        ttk.Button(self.unpack_tab, text="Browse", command=self.browse_unpack_file).grid(row=0, column=2, pady=10,
                                                                                         padx=10)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--platform", default="PC", choices=(
        "PC", "X360", "PS3", "ORBIS", "WII", "WIIU", "DURANGO", "NX", "GGP", "PROSPERO", "SCARLETT"),
                        help="Specify the platform (default: PC) *Pack only*")
    parser.add_argument("--game", type=int, default=2017,
                        choices=(2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022),
                        help="Specify the game version (default: 2017) *Pack only*")
    ...
    if args.pack:
        ...
        print(f"  File Count:        {header.files_count}")
        print(f"  Platform:          {header.platform}")
        print(f"  Engine:            {header.engine_version}")
        print(f"  Engine Signature:  {header.engine_signature}")
        print(f"  Engine Version:    {header.engine_version}")
        print(f"  Compressed:        {header.compressed}")
        print(f"  Binary Logic:      {header.binary_scene}")
        print(f"  Binary Logic:      {header.binary_logic}")
